...
---
# Evaluierung
26.06.2018, 10:00 - 12:00 Uhr

http://r.sd.uni-frankfurt.de/85825621
...
Pixel pro Monat gebrannt haben. Die Gesamtanzahl der brennenden Pixel pro Monat wird im data.frame tsFire, die Anzahl der Pixel pro Landbedeckungsklasse im data.frame tsLCTFire gespeichert.
Die Schleife dauert recht lange, verlieren Sie nicht
...
Wie häufig brennt es in der Region Ihrer Wahl? Erstellen Sie mit ggplot ein Histogramm für die Variable count im dataframe tsFire. Benutzen sie dafür geom_histogram.
```{r, eval=FALSE}
p <- ggplot(data=??, aes(??))
p <- p + geom_histogram(??)
```
Wechseln Sie eventuell die Region fall Sie feststellen dass es sehr wenig brennt.

Erzeugen Sie eine PDF Datei, mit der Zeitreihe:

\marginnote{
\textbf{Aufgabe}\\
Welche Einheit muss auf der y-Achse stehen?

Überlegen Sie, wie Sie das als Anteil in Prozent der gesammten
...
Um heraus zu finden, was gebrannt hat, wurden die verbrannten Pixel auch
nach Landbedeckungsklasse getrennt gespeichtert. Plotten Sie jetzt die Zeitreihen
getrennt für jede Landbedeckungsklasse. Ergänzen Sie dafür den Befehl facet_wrap im code.
...
\marginnote{
\textbf{Aufgabe}\\
Füre eine Animation sind die Datenmengen sehr groß definieren Sie daher einen Bereich (fireExtent) auf den die Bilder zugeschnitten
werden (crop), um die Datenmengen zu reduzieren. Testen Sie dafür einige Monate 
und benutzen Sie den plot(rFire) Befehl um schnell zu sehen ob und wo es brennt.
}
```{r, eval=FALSE}
...
plot(rFire)
```
Für die Beschriftung der einzelnen Zeitschritte extrahieren wir das Datum aus den Dateinamen.
```{r, eval=FALSE}
...
Jeder Zeitschritt wird einzeln eingelesen und in einer Liste gespeichert, die data.frames dieser liste werden im Anschluss aneinander gefügt. Wählen Sie einen kleineren Bereich, indem Sie fireExtent neu definieren, wenn es mit der Animation zu lange dauert.
...
p <- p + ylab("northing")
p <- p + labs(title = "Fire ")
gganimate(p,file='Fire.html')

```


# Aufgaben
1. Erstellen Sie eine Abbildung der mehrjährigen Monatsmittel der Feuerdynamik (basierend auf tsFire)! Gehen Sie dabei analog zur Hausaufgabenlösung des Phänologie Handouts vor. **(3 Punkte)**
...
3. Beschreiben Sie was der Aufruf geom_raster(aes(fill=state)) und der Aufruf geom_path(data=regionmap, size=0.1, colour="black", aes(x=long, y=lat, group=group)) zu dem Plotobjekt hinzufügt. **(2 Punkte)**
