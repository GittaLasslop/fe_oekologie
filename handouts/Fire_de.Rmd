...
Die Ländergrenzen werden auf UTM umprojiziert.
\marginnote{
\textbf{Aufgabe}\\
...
Diese Bilder können anschließend in eine Animation umgewandelt werden (freiwillig).

```{r, eval=FALSE}
png(filename=file.path(getOption("fegloekFigDir"), "Fire_%03d.png"), width=800, height=600)
...
Die Einzelbilder können anschließend zu einem Film zusammengesetzt werden.
}
# Hausaufgaben
1. Wie können Sie eine Abbildung mit dem mittleren Jahresgang der Feuersaisonalität machen? Gehen Sie analog zur Hausaufgabenlösung des PHEN handouts vor.
2. Ist die maximale Feueraktivität für alle land cover typen im gleichen Monat? Um dies Abschätzen zu können, fertigen Sie eine Abbildung mit geom_line() der Zeitreihe an in der die Farbe der Linie den Landbedeckungstypen angibt. Da die Feueraktivität eventuell für die verschiedenen Landbedeckungstypen sehr unterschiedlich ist könnte es evetuell sinnvoll sein den Logarithmus der Feueraktivität abzubilden. Welche zusätzlichen Zeilen code und/oder welche Änderungen sind dafür notwendig?
3. 
