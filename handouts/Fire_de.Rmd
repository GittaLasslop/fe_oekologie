...
classoption: a4paper
lang: de-DE
bibliography: /home/jsteinkamp/Cloud/Dropbox/etc/texmf/bib/zotero.bib
output:
  tufte::tufte_handout:
...
MODIS Kürzel: [MCD45A1](https://lpdaac.usgs.gov/dataset_discovery/modis/modis_products_table/mcd45a1)
...
for (i in 1:length(files)) {
  ...
  dfFire = merge(dfFire, dfLCT, by=c("x", "y"))

  if (exists("tsLCTFire") && nrow(dfFire) > 0) {
    tsLCTFire <- rbind(tsLCTFire,
                       cbind(ddply(dfFire, c("name"), summarize,
                                   count=length(burndate)), date=date[i]))
  } else if (nrow(dfFire) > 0) {
    tsLCTFire <- cbind(ddply(dfFire, c("name"), summarize,
                             count=length(burndate)), date=date[i])
  }
  message(paste0(round(i / (length(files)) * 100, 1), "% "))
}
```
...
Die Einzelbilder anschließend können zu einem Film zu zusammengesetzt werden.
}
