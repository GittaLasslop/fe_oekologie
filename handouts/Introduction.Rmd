...
Speichern Sie das 5. bis 8. Element der Variablen x in die Variable y.
-------------------------------------
...
Erstellen Sie einen *data.frame* "Tabelle" mit den Spalten "Latitude", "Longitude", "Landcover". Die Spalte "Landcover" soll den Typ Faktor haben.

lat = c(50.11, 23.9, 50.2)
lon = c(8.68, 32.9, 8.2)
lu  = c("urban", "water", "forest")

Führen Sie außerdem den Befehl *summary* aus: summary(Tabelle)
...
multidim
```
Beim Aufruf der Funktion werden erst die Werte dann die Dimensionen übergeben. Ein häufiges Beispiel für 3-d arrays sind Daten, die über Raum- und Zeitkoordinaten verfügen (x,y,t). Oft wollen wir dann beispielsweise den Mittelwert über die Zeit berechnen. Dies können Sie mit der Funktion *apply*, der Angabe der Dimensionen, die erhalten bleiben sollen (x,y) und der zu berechnenden Statistik (Mittelwert: *mean*) erreichen:   
```{r}
apply(multidim,c(1,2),mean)
```

-------------------------------------
**Aufgabe**

Berechnen Sie analog das räumliche Mittel.
-------------------------------------
