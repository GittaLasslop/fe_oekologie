...
```
Der Befehl ist etwas schwer verständlich, wir gehen ihn von rechts nach links durch. `Landcover==255` erzeugt ein bool raster (TRUE/ FALS raster) - alle Raster-Zellen mit Wert 255 werden TRUE, alle anderen werden FALSE. Mit der Funktion `is.na()` werden nun alle TRUE Positionen im Raster auf NA gesetzt.

```{r}
# Gitta, was haelst Du von der Alternative? Ist fuer mich verstaendlicher, is.na ist fuer mich eine Abfrage und bei einer Zuweisung verwirrend. Ich glaube der Sybtax kommt auch spaeter im LU/LUC Teil so vor...
