---
title: "Fernerkundung globale Ökologie: Landnutzungsänderung"
...
Für eine Diskussion von Unsicherheiten der genutzten Produkten:  
...
Laden Sie dort die Datei "LUC_{REGION}.zip" und speichern Sie sie in dem Ordner:

```{r, message=FALSE, warning=FALSE, eval=FALSE}
file.path(getOption("fegloekDataDir"), getOption("fegloekRegion"))
```

Ausserdem benötigen wir die Funktion *geotiff2df()* aus den letzten Sitzungen. Kopieren Sie den Quelltext der Funtion in dieses Script oder laden Sie die Funktion aus einer externen Datei *functions.R* (in ihrem OLAT_Skript haben wir die Funktion schon eingefügt - generell ist es aber ratsam bei größeren Projekten häufig verwendete Funktionen in externe Dateien auszulagern und mit source in the Laufzeitumgebung zu laden).
...
2. In dem Skript wird das Verhältnis Wald zu nicht-Wald in einem kleinem Ausschnitt betrachtet. Überlegen Sie sich eine eigene Klassifizierung (Urbanisierung, Landwitschaft, Verbuschung, ...) für Ihre Region und passen Sie die Variablennamen und die Klassifizierung entsprechend an. Eventuell muss auch der zu benutzende Ausschnitt verändert werden (forestExtent; schauen Sie sich den bisherigen Wert an um die Angabe zu verstehen), damit zum Beispiel eine Stadt enthalten ist. **(2 Punkte)**
...
4. Wählen Sie eine sinnvolle Farbgebung aus. Hilfe gibt es auf [docs.ggplot2.org](http://docs.ggplot2.org/current/) **(1 Punkt)**
