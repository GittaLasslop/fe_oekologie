...
```{r, eval=FALSE}
as.numeric(format(melted.dfNDVI\$date,"%m")). 
```
Jetzt müssen Sie die Variable monatsweise mitteln. Dies erreichen Sie mit der Funktion aggregate:
```{r, eval=FALSE}
aggregate(melted.dfNDVI,by=list(month=melted.dfNDVI\$month,
                                variablen=melted.dfNDVI$variable),
                                FUN='mean',na.rm=T)
```
 **(3 Punkte)**
 
3. Wiederholen Sie alle Abbildungen für eine andere Region aus der temperaten Zone **(1 Punkt)**.
